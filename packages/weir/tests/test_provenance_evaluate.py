...
def _run_provenance(
    *,
    ...
    joins = [
        JoinRecord(
            tool_call_source_ref="n0",
            tool_result_source_ref="n1",
...
def test_declared_provenance_flow_is_verdict_grade():
    findings = _run_provenance(
        ...
        result_tool="read_file",
        result_content=f"pay {_IBAN}",
        sink_tool="send_money",
        sink_args={"recipient": _IBAN},
    )
...
def test_no_provenance_rule_for_sink_means_no_finding():
    ...
    assert (
        len([f for f in findings2 if f.kind == "provenance"]) == 1
    )  # only the send_money rule fires
...
def test_double_fire_prefers_structural():
    ...
    verbatim_rule = RuleSpec(
        id="struct",
        version="1.0.0",
        stage="active",
        description="x",
        source_class="financial_account_identifier",
        sink_tool_name="send_money",
        mode="verbatim",
    )
    findings = _run_provenance(
        untrusted_sources=["read_file"],
        ...
        sink_tool="send_money",
        sink_args={"recipient": _IBAN},
        extra_sources=[ibansrc],
        extra_rules=[verbatim_rule],
    )
