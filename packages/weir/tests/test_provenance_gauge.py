...
def _node(i, kind, payload, degraded=False):
    return TraceNode(id=f"n{i}", kind=kind, timestamp=f"2020-01-01T00:00:00.{i:06d}Z",
                     actor="x", source_ref=f"n{i}", payload=payload, degraded=degraded)
...
def _two_readfile_one_lookup():
    ...
    def add_flow(tool, val):
        nonlocal idx
        c = _node(idx, NodeKind.TOOL_CALL, ToolCallPayload(tool_name=tool, args={})); ci = idx; idx += 1
        r = _node(idx, NodeKind.TOOL_RESULT, ToolResultPayload(content=f"pay {val}")); ri = idx; idx += 1
        s = _node(idx, NodeKind.TOOL_CALL, ToolCallPayload(tool_name="send_money", args={"recipient": val})); idx += 1
        joins.append(JoinRecord(tool_call_source_ref=f"n{ci}", tool_result_source_ref=f"n{ri}",
...
def test_declared_reports_yes():
    lines = provenance_gauge_lines(_two_readfile_one_lookup(),
        provenance_sink_names={"send_money"}, untrusted_sources=["read_file"])
